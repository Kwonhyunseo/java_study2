 ...
 *            => 한글자당 1byte 읽기/쓰기 =>한글은 꺠진다.
 *            => 업로드 , 다운로드 
 *     =2byte => ~Reader , ~ Writer (한글)          
 *            => 한글제어 
 *    -------------------                                      -----------------
 *     메모리 (데이터저장 )    ===========  자바응용 프로그램 ===========  모니터 출력 
 *     ------------------                                       ----------------
 *                              |                       |
 ...
 *                              OutputStream(바이트 스트림 :1byte)/Writer(문자스트림 : 2byte)
 *       IO관련 클래스 
 *             Object 
 *             |
 ...
 *                      업로드 /다운로드                   ------------------------------
 *  |                        |              |                           |         
 * ImputStream        OutputStream        Reader                       Writer         
 *     |                      |                |                         |
...
public class InputOuputMain {
	...
	new SimpelDateFormat("yyyy년 mm월 dd일 hh시 mm분 ss초").format()
		System.out.println("경로명 전용: "+file.getParent());
		System.out.println("파일명 전용: "+file.getName());
