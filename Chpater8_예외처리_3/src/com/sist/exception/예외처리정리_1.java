 ...
 *                               Exception/Throwable 
 *                               
 *      ****catch절은 순서가 존재한다 => 상위 클래스는 아래에ㅐ 배치한다
 ...
 *      ---------------------
 *      --------------------
 *      -------------------- 에러 발생 => 수행하지 않고 =>catch 로 이동
 *      ---------------------
 *      ---------------------
 *      
 *        }
 *        => catch는 위에서 한개씩 검색한후에 해당 예외처리클래스를 찾아 준다.
 *                  ------------------ 다중 조건문과 동일 (catch 는 한개만 수행)
 *    = 간접처리 (예외회피) : 자바나 오픈 라이브러리 
 *       예외에 대해 예측되는 예외클래스르르 선언만 한다
 ...
 *    ----------------------------------------------------------
 *    = 임의발생 : 테스트용으로 보통사용 
 *      throw new 예외처리클래스(); => 해당 예외처리클래스가 있는 catch 로 이동 
 *    = 사용자 정의 예외처리 
 *       예외처리 클래스를 상속 받아서 처리 
