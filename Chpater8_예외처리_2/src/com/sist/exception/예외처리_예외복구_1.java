 ...
 *                                         =>형변화, 파일명이 틀린경우
 *                                         =>Web에서 URL/IP틀린경우
 *                                         =>객체가 null일댸 주소에 접근하는 경우
 *                   Exception:예외처리의 최상위 클래스
 *                   ---------
 *                     |
 *                   1) CheckedEception : 컴파일시에 예외처리를 했는지 여부 확인 
 *                                         반드시 예외처리를 해야된다.
 *                   2)UnChehckException : 컴파일시에 예외처리를 확인하지 않는다.
 *                                         필요시에만 예외처리                      
 *                    Exception
 *                       |
 *   ---------------------------------------------------
 *   1) CheckedEception
 *              |
 *           =>IOExceptiom : 파일 입출력,메몰리 입출력, 네트워크 입출력
 *             (java.io)
 *           =>SQLException :오라클 연결(오라클리 인식하는 언어:SQL)
 *             JDBC ==>DBCP ==>ORM(MyBatis,JPA)
 *             Web =핵심(데이터베이스 연동)
 *             (java.sql)
 *           =>MalformedURLException :IP/Web URL  
 *            (java.net)
 *           =>ClassNotFoundException :클래스 이름으로 메모리 할당 => 클래스명,패키지명 틀린 경우
 *             => 스프링에서 주로 사용하는 기술 (리플렉션)
 *                                       -----private 까지 접근이 가능 
 *           A a =new A() ; Class.forName("A")
 *           =>InterrruptedException: 쓰레드(한개의 프로세스안에서 여러개의 프로그램을 동시에 사용)
 *              충돌과 관련 
 *           =>java.lang/java.util => 예외처리와 관련이 없다.
 *           =>예외처리는 필수 : 정상 사애를 유지=>견고한 프로그램
 *   2) UnCheckedException:예외처리 필요한 경우에만 사용 
 *           Exception
 *              |
 *             RuntimeException : 실행시 (인터프리터)=> 사용자 입력 ,프로그램 실수
 *              |
 *            NumberFormatException : 정수변환에서 에러 
 *              =>Wrapper : 기본형을 사용하기 쉽게 클래스 형으로 제작 
 *                          boolean =>Boolean 
 *                          int => Integer
 *                          double =>Double
 *               Interger.parseInt("10"); =>윈도우/Web(문자열만 사용)
 *                웹에서 전송된 데이터를 받는 경우
 *                -------------------------
 *               String  getParameter() => 정수
 *              Jsoup : 데이터 수집 (String)
 *         ArrayIndexOutOfBoundsExcepion: 배열범위 초과
 *         ------------------------------ 컬렉션 
 *         ArithmeticExceptiom : 0으로 나눌 경우 
