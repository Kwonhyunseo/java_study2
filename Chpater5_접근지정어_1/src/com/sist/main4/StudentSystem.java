...
public class StudentSystem {
	...
	public  void display() // default => com.sist.main4안에서만 접근이 가능 
	 ...
	 {             //com.sist.main5
		 for(Student s:std)
		 {
			 System.out.println(s.getHakbun()+ "" +s.name+ "" + s.getKor()+ "" +s.getEng()+"" +s.getSex()+"" + s,getTotal()+ "" + s.get);
		 }
	 }
	//2. 평균 구하기
	public void stdAvg()
   ...
   // 3. 총점 구하기
	...
	public void stdTotal()
	{
		for(int i=0;i< std .length;i++)
		{
			std[i].setTotal(std[i].getKor()+std[i].getEng()+std[i].getMath()+std[i].getTotal()+" ");
		}
	...
	//5. 등수 구하기
	 ...
	 
	 {
		 for(int i=0; i<std.length;i++)
		 {
			 ...
			 {
				 if(std[i]i.getTotal()<std[j]).getTotal())
{
	 std[i].setRank(std)
}
			 }
