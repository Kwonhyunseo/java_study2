 ...
 *              = set() => 년도/월/일/시간/분/초 
 *              = get() 
 *              = 요일 읽기 => get ( Calendear.DAY_OF_WEEK)
 *              = 각달의 마지막 날
 *                getActualMacimum() 
 *   = 배열을 대체 
 *      = 3대 클래스 
 ...
 *     List               Set           Map=>인터페이스
 *        |                 |              |
 *    ArrayList(0)      HashSet(0)      HashMap(0)
...
public class 라이브러리_Calendar {
 ...
 // 사용자로부터 년도/월 =>예약 / 체크인 , 체크아웃 =>달력 
		...
		//추상클래스는 미완성된 클래스( 메서드가 구현 이 안된 상태)
		//구현할 클래스를 이용한다, 싱크런 (newInstance(),getInstance())
		/*
		 * static => 클래스명. 메소드명()
		 * Math.random()
